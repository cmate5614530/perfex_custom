...
class Clients extends ClientsController
{
    ...
    //10.2 AG: added code to send mail notification to supplliers
    public function send_notify_mail_to_suppliers($request_id)
    {
        //get mail list of suppliers
        $this->db->select('email');
        $this->db->join(db_prefix() . 'pur_contacts', db_prefix() . 'pur_contacts.userid = ' . db_prefix() . 'pur_vendor.userid', 'left');
        $vendors = $this->db->get(db_prefix().'pur_vendor')->result_array();  
        acer_log('Clients.php: send_notify_mail_to_suppliers,');
        acer_log($vendors);
        //get url
        $url = site_url('purchase/vendors_portal/offer_price/'.$request_id);
        //load mail configuration
        $this->load->config('email');

        $template           = new StdClass();
        $template->message  = get_option('email_header') . _l('New_travel_request_posted'). '<br/>'
                            ._l('Please_check_details_and_make_offer').'<br/>'
        ...
        $template->subject  = _l('New_travel_request');
        ...
        foreach( $vendors as $vendor)
        {
            ...
            if ($this->email->send(false)) {
                //set_alert('success', 'Notified to correspoing suppliers via mail.');
            } else {
    ...
    //10.3 send push notification to suppliers
    public function send_notification_to_suppliers($request_id)
    {
        acer_log('Client.php send_notification_to_suppliers:start');
        //get userid for suppliers
        $this->db->select('userid');
        $vendors = $this->db->get(db_prefix().'pur_vendor')->result_array();  
        acer_log('Client.php send_notification_to_suppliers:');
        acer_log($vendors);
        //get url
        $url = site_url('purchase/vendors_portal/offer_price/'.$request_id);
        //send notification
        ...
        foreach ($vendors as $vendor)
        {
            ...
            //     'link' => 'clients/client/' . 1 . '?group=attachments',
            // ])) 
            {
                array_push($notifiedUsers, $vendor['userid']);
            }
        }
        pusher_trigger_notification($notifiedUsers, $url);
    }
    // 9.22 AG: added for a request 
    public function requests()
    {
        

        $client_user_id = get_client_user_id();
        $requests = $this->requests_model->get_requests_by_client( $client_user_id );
    ...
    // 9.25 AG: added for a request 
    public function offers($request_id)
    {
        $client_user_id = get_client_user_id();
        $offers = $this->requests_model->get_offers_by_vendor_request( '', $request_id );
        $data['offers']       = $offers; 
        //acer_log('clients.php: offers');
        //acer_log($offers);
        $this->data($data);
        $this->view('offers');
        $this->layout();
    ...
    //9.25 AG: accept offer
    public function accept_offer($offer_id)
    {
        //acer_log('clients.php: accept_offer');
        $invoiceid = $this->requests_model->convert_to_invoice($offer_id);
        $this->load->model('invoices_model');
        //acer_log('accept_offer: invoice_Id='.$invoiceid);
        if ($invoiceid) {
            ...
            // ]));
            set_alert('success', _l('clients_offer_invoiced_successfully'));
            $redURL = site_url('invoice/' . $invoice->id . '/' . $invoice->hash);
        }
        redirect($redURL);
    }

    //9.29 AG: cancel offer
    public function cancel_offer($offer_id)
    {
        //acer_log('clients.php: cancel_offer');
        $this->db->where('id', $offer_id);
        $this->db->update(db_prefix().'offers', [
            'status' => 'cancelled',
        ]);
        set_alert('warning', _l('clients_offer_cancelled'));
        redirect(site_url('clients/requests'));
    }
    ...
    {
        if ($this->input->post()) {
            //acer_log('new_reqeust');
            //acer_log($this->input->post());
            $this->form_validation->set_rules('pickup', _l('From'), 'required');
            $this->form_validation->set_rules('dropoff', _l('To'), 'required');
            $this->form_validation->set_rules('pickup_dt', _l('Pickup_Dt'), 'required');
            $this->form_validation->set_rules('passengers', _l('Passengers'), 'required');
            $this->form_validation->set_rules('vehicle_type', _l('Vehicle_Type'), 'required');
            if ($this->form_validation->run() !== false) {
                acer_log('form_validation: run, ture');
                $data = $this->input->post();

                $id = $this->requests_model->add($data);

                if ($id) {
                    set_alert('success', _l('new_request_added_successfully', $id));
                    $this->send_notify_mail_to_suppliers($id);
                    $this->send_notification_to_suppliers($id);
                    //redirect(site_url('clients/requests/' . $id));
                    redirect(site_url('clients/requests'));
                }
            }
        }
        ...
        $vehicle_types      = $this->db->get()->result_array();
        $data = [];
        $data['title']    = _l('New_Request');
        $data['vehicle_types']    = $vehicle_types ; 
        $this->data($data);
        $this->view('new_request');
        $this->layout();
    }
    //9.22 AG: end
